...
import { createCliRuntimeClient } from "../claude/cli-runtime-client";
...
export function createRuntimeDeps(
  config: AppConfig,
  overrides: {
    createCliRuntimeClientFn?: typeof createCliRuntimeClient;
  ...
  const claude = (overrides.createCliRuntimeClientFn ?? createCliRuntimeClient)(
    config.workspaceDir,
    {
  ...
  const cronRuntimeArgs: CronRuntimeWiringArgs = {
    claudeClawHomeDir: resolveClaudeClawHomeDir(),
