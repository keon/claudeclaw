...
export function createCronRuntime({
  ...
  const reportBackgroundError =
    onBackgroundError ??
    ((error: unknown) => {
      console.error("[claude-claw] cron background tick failed", error);
    });
  const dispatchWithClaude = async (prompt: string): Promise<CronDispatchResult> => {
    if (dispatchPrompt) {
    ...
    if (!claude) {
      throw new Error("createCronRuntime requires either dispatchPrompt or claude");
    }
  ...
  const dispatchCronPrompt = async (prompt: string): Promise<CronDispatchResult> => {
    ...
    return dispatchWithClaude(prompt);
  };
  ...
  async function loadSpecs(): Promise<{ specs: ScheduledJobSpec[]; errors: ScheduledJobIssue[] }> {
    const detected = await detectScheduledJobDefinitions({ claudeClawHomeDir });
