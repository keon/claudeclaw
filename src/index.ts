...
export async function main(): Promise<void> {
  ...
  bot.catch((error) => {
    console.error(`[claude-claw] update ${error.ctx.update.update_id} failed`, error.error);
  });
  ...
  if (telegramToken.source === "prompt") {
    console.info(`[claude-claw] saved TELEGRAM_BOT_TOKEN to ${localConfigStore.path}`);
  }

  console.info(`[claude-claw] starting Telegram bot with workspace ${config.workspaceDir}`);
  await handlers.startBackgroundServices();

  try {
    await bot.start({
      onStart(botInfo) {
        console.info(`[claude-claw] polling as @${botInfo.username}`);
      },
    });
  } finally {
...
if (import.meta.main) {
  await main().catch((error) => {
    console.error("[claude-claw] fatal startup error", error);
    process.exitCode = 1;
  });
}
